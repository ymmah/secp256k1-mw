...
static int secp256k1_bulletproof_rangeproof_verify_impl(const secp256k1_ecmult_context *ecmult_ctx, secp256k1_scratch *scratch, const unsigned char **proof, const size_t n_proofs, const size_t plen, size_t nbits, const secp256k1_ge **commitp, size_t n_commits, const secp256k1_ge *genp, const secp256k1_ge *gens, const size_t n_gens, const unsigned char **extra_commit, size_t *extra_commit_len) {
    ...
    ret = secp256k1_bulletproof_inner_product_verify_impl(ecmult_ctx, scratch, gens, n_gens, nbits * n_commits, innp_ctx, n_proofs, plen - (64 + 128 + 1));
    secp256k1_scratch_deallocate_frame(scratch);
    return ret;
}
 ...
 */
static int secp256k1_bulletproof_rangeproof_prove_impl(const secp256k1_ecmult_gen_context *ecmult_gen_ctx, const secp256k1_ecmult_context *ecmult_ctx, secp256k1_scratch *scratch, unsigned char *proof, size_t *plen, const size_t nbits, const uint64_t *value, const secp256k1_scalar *blind, const secp256k1_ge *commitp, size_t n_commits, const secp256k1_ge *genp, const secp256k1_ge *gens, const size_t n_gens, const unsigned char *nonce, const unsigned char *extra_commit, size_t extra_commit_len) {
    secp256k1_bulletproof_lr_generator lr_gen;
    ...
    for (i = 0; i < n_commits; i++) {
        for (j = 0; j < nbits; j++) {
            ...
            secp256k1_ge aterm = gens[i * nbits + j + n_gens/2];
            secp256k1_ge sterm;
            ...
            secp256k1_fe_cmov(&aterm.x, &gens[i * nbits + j].x, al);
            secp256k1_fe_cmov(&aterm.y, &gens[i * nbits + j].y, al);

            secp256k1_gej_add_ge(&aj, &aj, &aterm);

            secp256k1_ecmult_const(&stermj, &gens[i * nbits + j], &sl, 256);
            secp256k1_ge_set_gej(&sterm, &stermj);
            secp256k1_gej_add_ge(&sj, &sj, &sterm);
            secp256k1_ecmult_const(&stermj, &gens[i * nbits + j + n_gens/2], &sr, 256);
            secp256k1_ge_set_gej(&sterm, &stermj);
            secp256k1_gej_add_ge(&sj, &sj, &sterm);
        }
    }
    ...
    *plen -= 64 + 128 + 1;
    ...
    if (secp256k1_bulletproof_inner_product_prove_impl(ecmult_ctx, scratch, &proof[64 + 128 + 1], plen, gens, n_gens, &y, nbits * n_commits, secp256k1_bulletproof_abgh_callback, (void *) &abgh_data, commit) == 0) {
        return 0;
    }
    *plen += 64 + 128 + 1;
